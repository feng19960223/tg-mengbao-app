...
import com.turingoal.mengbao.ui.adapter.AttendanceAdapter;
                ...
                if (result.isSuccess()) {
                    ...
                    if (attendances != null) {
                        if (TgConstantGetDataType.INIT == getDataType) {
                            ...
                            attendanceList.removeAll(mAdapter.getData());
                            mAdapter.addData(attendanceList);
                        }
                        mAdapter.loadMoreComplete(); // 成功获取更多数据
                        if (attendances.size() < limitSize) {
