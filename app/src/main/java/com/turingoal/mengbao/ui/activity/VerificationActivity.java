 ...
 * 验证码
 */
@Route(path = ConstantActivityPath.VERIFICATION)
public class VerificationActivity extends TgBaseActivity {
    ...
    protected void initialized() {
        ...
        tvTitle.setText(R.string.activity_verification);
    ...
    private void verification() {
        PostRequest request = TgHttpUtil.requestPost(ConstantUrls.URL_PASSWORD_VERIFICATION, getHttpTaskKey());
        request.params("phone", phone);
        ...
        request.execute(new TgHttpCallback(null) {
            ...
            public void successHandler(final TgResponseBean result) {
                ...
                if (results) {
                    Toast.makeText(VerificationActivity.this, getString(R.string.verification_success), Toast.LENGTH_SHORT).show();
                    TgSystemHelper.handleIntentWithObj(ConstantActivityPath.NEW_PASSWORD, "phone", phone); // 设置新密码界面
                } else {
                    Toast.makeText(VerificationActivity.this, getString(R.string.verification_error), Toast.LENGTH_SHORT).show();
                }
            }
        });
    }
